...
const CASES := [
	["_grant_true_dawn_recipe", "WarmDoorEntrance", &"ending_04_true_dawn"],
	["_grant_forest_recipe", "RootNetworkEntrance", &"ending_05_autonomous_forest"],
	["_grant_growth_recipe", "BarcodeTerminalEntrance", &"ending_06_infinite_growth_inc"],
	["_grant_frost_recipe", "FreezerAlcoveEntrance", &"ending_07_eternal_winter_seed"],
	["_grant_root_recipe", "TemperatureProbeEntrance", &"ending_08_root_access"],
]
...
func _ready() -> void:
	...
	for index in range(CASES.size()):
		...
		if debug_panel == null:
			_fail("第 %d 条路线缺少 DebugPanel" % (index + 4))
			return
		...
		if index < CASES.size() - 1:
			main._start_new_run(3200 + index)
			await get_tree().process_frame
			await get_tree().physics_frame
	print("PHASE 20 ENDINGS FLOW PASS: 结局 04–08 均能从调试配方经实际入口进入结算并写入图鉴。")
	get_tree().quit(0)


func _fail(message: String) -> void:
	push_error(message)
	print("PHASE 20 ENDINGS FLOW FAIL: %s" % message)
	get_tree().quit(1)
