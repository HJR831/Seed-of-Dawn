...
第 0–13 小时使用的 1280×3300 灰盒只用于验证核心流程，不是最终地图尺寸。最终主关卡建议扩展到约 2560×7200 至 3200×9000；具体数值可以随美术调整，但可探索宽度必须明显大于单个屏幕，不能只把现有竖直走廊机械拉长。

参考分区以 8000 px 高地图为例：

```text
y = 6500–8000  出生点 / 腐土深渊
y = 4800–6500  黄瓜、黏液、现实线索
y = 3000–4800  保鲜膜、同伴与返回下层通路
y = 1200–3000  辣酱瓶、冰霜、温控线路
y = 0–1200     盒盖、灯开关、门缝及特殊出口
```

数字只是灰盒参考，美术到位后再调。所有隐藏路线在同一张地图中通过侧室、回环、返回下层和特殊交互实现，不制作十二张地图。

地图扩展必须遵守：

- 每一幕至少有一条直达主路线、两处可选侧室，以及一条能回到主路线的回环；避免所有支路都是拿完道具后原路返回的死胡同。
- 可选区域占可行走空间约 30%–50%，隐藏结局所需物品分散在不同方向，让玩家产生“我还可以去别处看看”的选择。
- 连续无互动的直线移动不超过约 10 秒；每 20–40 秒出现清楚地标，每 45–90 秒至少出现一次交互、线索、路线选择或材质变化。
- 主路线实际移动轨迹建议约 10000–16000 px；全部支路总长度约为主路线的 1.5–2 倍。增加时间应来自判断、发现和操作，而不是重复减速或空走。
- 同一幕至少设置两个远距离可见的方向提示，例如瓶身高光、冷风粒子、灯线或颜色变化，降低大地图中的迷路风险。
- 出生点、幕间门、特殊出口和回程捷径使用 `Marker2D` 或独立配置节点管理；不要继续把所有位置写死在 `fridge_level.gd` 常量中。

最终游玩时长使用 `GameState.run_seconds` 和 `input_distance` 实测：第一次不知道路线的玩家到默认结局目标为 10–20 分钟；熟悉路线后直达默认结局约 5–8 分钟；主动寻找隐藏结局可以超过 20 分钟。若测试者用时不足，优先增加有选择的支路与互动，不添加纯空白走廊。
...
- [ ] 先把主关卡坐标框架扩展到至少 2560×7200，并完成各幕主路线、侧室、回环和 `Marker2D` 传送点；第 0–13 小时小地图保留在 Git 历史中，不另建第二套正式关卡。
- [ ] 将黏液、冰霜、硬障碍、保鲜膜、拾取物和叙事触发按探索节奏重新分布，禁止用大片空走区域填充时长。
- [ ] 结局 01 默认土豆。
...
验收：前三结局从同一张扩展关卡稳定触发，重开后图鉴显示 3/12；熟悉路线直达默认结局不少于约 5 分钟，盲测玩家能通过地标找到主路线。
