...
第 0–13 小时的 1280×3300 灰盒和第 13–20 小时的 2560×7600 纵向扩展图都属于已完成的集成原型，用于验证移动、三条结局与存档；它们保留在 Git 历史中，但不作为最终地图形态。最终主关卡改为“从冰箱内部核心向外探索”的近圆形结构，逻辑边界至少 8192×8192，出生点位于中心附近，可探索面积必须明显大于当前版本。

推荐按距中心的半径划分生态环：

```text
r = 0–700       出生核心 / 母薯安全区
r = 700–1700    腐土、黄瓜与现实线索环
r = 1700–2800   保鲜膜、同伴与养分环
r = 2800–3800   泰坦赤柱、冰霜与温控环
r = 3800–4400   外壳、盒盖、门缝及特殊出口环
```

圆形不是一条完美的环形走廊。每局应生成 8–12 个角向扇区，以径向通道负责向外推进，以切向回环连接相邻扇区，再加入侧室、近路和能够回到内环的捷径。玩家可以选择螺旋探索、跨扇区绕行或返回中心，至少存在两条大体独立的中心到外缘路线；所有结局仍复用同一张逻辑地图，不制作十二张地图。

地图、区域和道具使用受约束的种子化生成，不做完全无规则随机：

- 每局生成并在 `GameState`、Debug 面板中记录 `run_seed`；相同种子必须复现相同房间连接、区域与道具位置，方便复现 Bug。
- `procedural_map_generator.gd` 先生成可行走图、扇区房间、门和捷径，再做连通性验证，最后才放置黏液、冰霜、仪式、出口、线索和拾取物。
- 每类区域和物品声明允许出现的生态环、扇区数量、与同类/出生点/主路线的最小距离。随机性改变扇区旋转、连接方式和具体摆放，不改变 `item_id`、结局条件或配方。
...
- 地标的类别、色彩与生态环含义保持稳定，即使位置随机也能被学习；HUD 提供简洁的径向深度/罗盘提示，不直接显示隐藏道具。

建立固定种子回归集，至少批量检查 100 个种子的连通性、关键物品顺序、碰撞重叠、仪式净空和生成耗时。最终游玩时长使用 `GameState.run_seconds` 和 `input_distance` 实测：第一次不知道路线的玩家到默认结局目标为 10–20 分钟；熟悉布局规则后直达默认结局约 5–8 分钟；主动寻找隐藏结局可以超过 20 分钟。

### 7.2 Camera2D

- 相机跟随芽尖，Position Smoothing 开启。
- 四向范围限制在本局圆形地图的外接矩形内；外缘使用实体与遮罩阻止玩家进入无内容区域。
- HUD 可显示玩家相对中心的方向与所处生态环，但不显示完整战争迷雾地图。
...
验收：前三结局从同一张纵向集成关卡稳定触发，重开后图鉴显示 3/12；本阶段重点验证系统与配方，最终 10–20 分钟游玩时长由后续圆形程序化地图实现。

### 第 20–32 小时：第二批五结局

- [x] 建立 `procedural_map_generator.gd`，把现有关卡迁移为至少 8192×8192、中心出生、向外扩展的生态环与 8–12 个扇区。
...
- [ ] 最终地图从中心向外生成；固定 100 个种子的连通性、关键物品顺序、碰撞重叠和仪式净空测试通过，失败生成能安全回退到固定布局。
- [ ] 本局重开后行为状态清空，永久图鉴保留。
- [ ] 文本不会重叠，结局演出期间输入被锁定。
- [ ] 音乐、环境声和音效可分别调节。
...
## 15. 第 13–20 小时版本完成后的五件事

1. 重新测试赤柱逆时针仪式，确认上方紫色检查点可进入，四点顺序能够完成。
2. 冻结当前纵向扩展图为第 13–20 小时集成基线；后续在独立生成器中迁移内容，不一边扩建旧坐标一边修改结局逻辑。
3. 先完成 `run_seed`、生态环/扇区图结构、连通性验证和固定后备布局，再把区域、道具与五条新结局接入随机生成。
4. 实现物品栏/背包并接入 `item_text.csv`，保证它只读取 GameState，本局重开后清空。
5. 用固定 100 个种子跑自动测试，再邀请未看路线的玩家进行 10–20 分钟盲测。

之后你可以直接这样下达任务：

> 请按照 programming.md 的第 20–32 小时阶段继续制作。先实现中心向外的圆形程序化地图和物品栏，再接入结局 04–08；完成后运行固定种子、结局判定与存档回归测试，并告诉我如何在 Godot 中试玩。
